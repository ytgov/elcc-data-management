<template>
  <v-dialog v-model="visible" persistent max-width="500">
    <v-card v-if="editingCentre">
      <v-toolbar color="primary" variant="dark" title="Centre Edit">
        <v-spacer></v-spacer>
        <v-btn icon @click="close" color="white"><v-icon>mdi-close</v-icon></v-btn>
      </v-toolbar>
      <v-card-text>
        <v-text-field label="Name" v-model="editingCentre.name" variant="outlined" density="comfortable"></v-text-field>
        <v-text-field
          label="License"
      ...
      <v-card-actions class="mx-4 mb-2">
        <v-btn color="primary" variant="flat" @click="saveClick" :disabled="!canSave">Save</v-btn>
        <v-spacer></v-spacer>
